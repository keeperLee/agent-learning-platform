  ...
  entries: [
    {
      ...
      items: [
        ...
        { type: 'fix', text: '修复校验脚本在 Windows 上会静默失效的问题。章节文件在 Windows 检出时是 CRLF，而逐行扫描用的 `(.*)$` 中 `.` 不匹配 `\\r`、`$` 又要求字符串结尾，于是 `:::demo / :::diagram / :::callout` 的名称校验**只在 Linux CI 上真正运行**，本地一直跳过（表现为统计表里「演示」列恒为 0）。现在读取时统一换行符，两边行为一致' },
