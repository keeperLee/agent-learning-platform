...
test('SQLite authentication lifecycle and HTTP isolation',async()=>{
 ...
 try {
  ...
  if(pub){
    // 公开访问：没有 Cookie 也应该能拿到门户与课程内容
    assert.equal(await status('/learning.html'),200);
    assert.equal(await status('/content/projects.js'),200);
    assert.equal(await status('/content/chapters/c01.md'),200);
  }else{
    const redirect=await fetch(base+'/learning.html',{redirect:'manual'});
  ...
  if(!pub)assert.equal(await status('/content/chapters/c01.md'),401);
  ...
  for(const path of ['/', '/learning.html', '/content/projects.js']) assert.equal((await fetch(base+path,{headers:{Cookie:admin},redirect:'manual'})).status,200);
  const portal=await (await fetch(base+'/')).text();assert.match(portal,/id="loginForm"/);
  assert.equal((await fetch(base+'/content/chapters/c01.md',{headers:{Cookie:admin}})).status,200);
  ...
  assert.equal(await status('/learning.html',{headers:{Cookie:admin}}),pub?200:302);
