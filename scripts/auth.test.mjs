...
test('SQLite authentication lifecycle and HTTP isolation',async()=>{
 ...
 try {
  ...
  assert.equal((await fetch(base+'/content/chapters/c01.md')).status,401);
  ...
  assert.equal((await fetch(base+'/learning.html',{headers:{Cookie:admin},redirect:'manual'})).status,302);
